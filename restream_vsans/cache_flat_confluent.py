...
CONFLUENT = 0
if CONFLUENT:
    # confluent kafka is failing for no good reason so don't use it
    # maybe it has to do with not closing and freeing object wrappers
    # Presently unusable, even for getting a timing estimate
...
class EventDatabase:
    ...
    def __init__(self, path, mode='w'):
        ...
        #self.file.create_dataset('version', data=(self.version,), dtype='i4')
        #self.file.create_dataset('start', shape=(1,), dtype='i8')
        #self.file.create_dataset('stop', shape=(1,), dtype='i8')
        self._fields = {}
...
def stream_history(consumer, topic, start, stop, partitions=None, timeout_ms=100):
    ...
    for pid in partitions:
        ...
        if 1:
            ...
            earliest, latest = consumer.get_watermark_offsets(partition_handle)
            print(f"partition[{pid}] {earliest=} {latest=}")
            print("seeking", TopicPartition(topic, pid, earliest))
            consumer.seek(TopicPartition(topic, pid, earliest))
            earliest = consumer.poll().timestamp()[1]
            consumer.seek(TopicPartition(topic, pid, latest-1))
            latest = consumer.poll().timestamp()[1]
            print(f"partition[{pid}] {earliest=} {latest=}")
        else:
        ...
        if CONFLUENT:
           part = consumer.offsets_for_times([TopicPartition(topic, pid, start)])
           offset = part[0].offset
        ...
        consumer.seek(TopicPartition(topic, pid, offset)) if CONFLUENT else consumer.seek(partition_handle, offset)
        # Single partition so messages are guaranteed to be in timestamp order
        #print("reading messages")
        while CONFLUENT: # CONFLUENT
            message = consumer.poll()
            #print("loop", message, message.timestamp(), stop)
            if message.timestamp()[1] >= stop:
                break
            else:
                result = Message()
                result.timestamp = message.timestamp()[1]
...
if CONFLUENT:
    class Message:
        __slots__ = ['timestamp', 'value', 'partition', 'topic']
