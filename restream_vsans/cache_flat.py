...
GATE_ON, GATE_OFF, TRIGGER_T0 = 1, 2, 3
class EventDatabase:
    """
    ...
    def trigger(self, timestamp, flag):
        if flag == TRIGGER_T0:
            self._create_or_extend_timestamp('T0', (timestamp,))
        elif flag == TRIGGER_GATE_OPEN:
            self._create_or_extend_timestamp('open', (timestamp,))
        elif flag == TRIGGER_GATE_CLOSE:
            self._create_or_extend_timestamp('close', (timestamp,))
	else:
	    logging.warn(f"Unknown trigger {flag} in {str(self._root)} at time {timestamp}")
...
def process_trigger(message, db):
    ...
    db.trigger(timestamp, trigger)
    # TODO: for now abuse GATE_OPEN/GATE_CLOSE as if they were ARM/DISARM signal
    # Pause/resume will show up as gate close followed by gate open without
    # arm/disarm. Instead the arm/disam will come through the nice topic for
    # start/stop count, with no notice of arm/disarm for the detector.
    if trigger == ARM:
        db.arm(timestamp)
    elif trigger == DISARM:
        db.disarm(timestamp)
