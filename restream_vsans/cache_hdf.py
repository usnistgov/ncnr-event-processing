...
class EventDatabase:
    ...
    def arm(self, timestamp):
        ...
        self.file['start'][0] = timestamp

    def disarm(self, timestamp):
        self.stop = timestamp
        self.file['stop'][0] = timestamp
        self._closing = True
    ...
    def _create_or_extend_timestamp(self, name, events):
        ...
        if name not in self.file:
            ...
            group.create_dataset('timestamp', data=timestamp, maxshape=(None,))
        else:
            n = len(timestamp)
            group = self.file[name]
            #print(group, n, group['timestamp'].shape)
            group['timestamp'].resize((group['timestamp'].shape[0]+n,))
            group['timestamp'][-n:] = timestamp
    ...
    def _create_or_extend_pairs(self, name, events, field, dtype):
        ...
        if name not in self.file:
            group = self.file.create_group(name)
            group.create_dataset('timestamp', data=timestamp, maxshape=(None,))
            group.create_dataset(field, data=values, maxshape=(None,))
        else:
            n = len(timestamp)
            group = self.file[name]
            group['timestamp'].resize((group['timestamp'].shape[0]+n,))
            group[field].resize((group[field].shape[0]+n,))
            group['timestamp'][-n:] = timestamp
            group[field][-n:] = values
...
def cache_filename(instrument, timestamp):
    ...
    filename = dt.strftime("%Y%m%d%H%M%S%f.hdf")
    return filename
