...
COMMIT;""")
    ...
    def monitor(self, neutrons):
        self.cursor.executemany(f"INSERT INTO monitor VALUES(?, ?)", neutrons)
        self.cursor.commit()
...
def setup():
    global DETECTOR_SCHEMA, TIMING_SCHEMA, DEVICE_SCHEMA, METADATA_SCHEMA
...
def process_detector(message, db):
    record = decoder(message, DETECTOR_SCHEMA)
    neutrons = ((n['timestamp'], n['pixel_id']) for n in record['neutrons'])
    db.detector(neutrons, message.partition)

def process_monitor(message, db):
    record = decoder(message, DETECTOR_SCHEMA)
    neutrons = ((n['timestamp'], n['pixel_id']) for n in record['neutrons'])
    db.monitor(neutrons, message.partition)
...
def process_message(message, db):
    if db is not None:
        processor = PROCESSOR[message.topic]
...
def stream_history(consumer, topic, start, stop, partitions=None, timeout_ms=100):
    ...
    for pid in partitions:
        partition_handle = TopicPartition(topic, partition)
        consumer.assign([partition_handle])
        offset = consumer.offsets_for_times({partition_handle: start})[partition_handle]
        consumer.seek(partion_handle, offset)
        # Single partition so messages are guaranteed to be in timestamp order
        while True:
            batches = consumer.poll(timeout_ms=timeout_ms)
            if not batches:
                return
            messages = batches[partition]
...
def fetch_events_for_file(filename, timeout_ms=100):
    ...
    nexus = h5py.File(filename)
    entry = nexus.children[0] # first entry
    ...
    start_count, stop_count = parse_timestamp(entry['start'].value), parse_timestamp(entry['stop'].value)
    instrument = entry['instrument'].value.decode('utf8')
    # TODO: use nexus filename plus point number for easier file management
    filename = f"{instrument}_{start_count}.db"
...
def live_stream(instrument, sync=1000):
    ...
    consumer.subscribe(pattern=f'{instrument}_.*')
    ...
    while True:
        ...
        for m in messages:
            if m.topic == trigger_topic:
                ...
                record = decode(m, TRIGGER_SCHEMA)
                ...
                if record.trigger == ARM:
                    ...
                    db = EventDatabase(CACHE_ROOT / current_file)
...
def main():
    ...
    setup()
    CACHE_ROOT.mkdir(parents=True, exists_ok=True)
